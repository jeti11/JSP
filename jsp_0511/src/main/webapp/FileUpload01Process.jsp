...
<%-- 1. cos.jar 라이브러리--%>
<%-- 2. apache의 commons.io, upload 라이브러리--%>

<%-- 1. cos.jar 사용시 파일 업로드 처리 --%>
<%-- MultipartRequest 클래스가 업로드 된 파일 정보를 처리--%>
<%-- 웹페이지에서 서버로 전송된 파일 자체만 다루는 클래스--%>
<%-- 클라이언트에서 multipart/form-data 유형과 post 방식의 요청 파라미터 등을 분석한 후 일반 데이터와 파일 데이터를 구분하여 파일 데이터에 접근할 수 있게 해줌--%>
...
<%-- maxPostSize : 세번째 매개변수, 서버로 전송된 파일의 최대 크기 설정, <byte 단위>--%>
<%-- encoding : 네번째 매개변수, 인코딩 유형 설정--%>
<%-- policy : 다섯번째 매개변수, 파일명 변경 정책 설정, 미설정시 동일한 파일 덮어쓰기--%>
...
<%-- getFile() : 전송된 파일에 대한 파일 객체를 출력, 파일이 없으면 null, java.io.File 타입으로 출력--%>
<%-- getFileNames() : 클라이언트의 form 태그에서 type이 file로 설정된 태그의 파라미터 명을 가져와서 Enumeration 타입으로 출력--%>
...
//  클라이언트의 form에서 전송된 input 태그의 name값을 모두 가져옴
//  Enumeration : 배열하고 비슷한 타입
...
//  클라이언트의 form에서 전송된 file의 파라미터명을 가져옴
