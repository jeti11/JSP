...
<%--데이터베이스 사용을 위해 DTO클래스와 DAO 클래스를 가져옴--%>
...
<%
  ...
  // 원래는 세션 정보에 저장된 userId를 불러와서 사용해야 함
  dto.setPostWriteUser((String)session.getAttribute("userId"));
  
  // 데이터 베이스 이용을 위해서 DAO 클래스 타입의 객체 생성 및 DB 연결
  BoardDAO dao = new BoardDAO();
  dao.dbOpen();
  
  // DAO 객체에서 제공하는 insert 메소드를 사용하여 데이터베이스에 글 등록
  ...
  if (insertResult == 1) {
    ...
    script += "alert('글 등록 중 오류가 발생했습니다.')";
    script += "history.back();";
    script += "</script>";
