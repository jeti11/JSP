...
@WebServlet("/blog/dailyComment.do")

public class DailyCommentController extends HttpServlet {
  @Override
  protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
    int no = Integer.parseInt(req.getParameter("no"));
    int idx = Integer.parseInt(req.getParameter("idx"));

    CommentDailyDAO dao = new CommentDailyDAO();
    int result = dao.commentNoDelete(no);
    dao.dbClose();

    if (result == 1) {
      JSFunc.alertLocation(resp, "선택하신 댓글이 삭제가 완료되었습니다.", "/blog/dailyView.do?idx=" + idx);
    }
    else {
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
    ...
    resp.sendRedirect("/blog/dailyView.do?idx=" + idx);
  }
}
